...
export const updateBlockPosition: any =
  (payload: any, source: any, destination: any, currentItem: any) =>
  async (dispatch: AppDispatch, getState: any) => {
    const prevIssueId =
      payload.columns[destination.droppableId].blockIds[destination.index - 1];
    const nextIssueId =
      payload.columns[destination.droppableId].blockIds[destination.index + 1];
    ...
    const nextPayload = createNextState(payload, (draftState: any) => {
      draftState.blocks[currentItem.id] = {
        ...currentItem,
    ...
    await fetch('http://localhost:8000/api/v1/issues/position', {
      method: 'PUT',
      headers: {
